...
class ShotMode(BaseMode):
    ...
    def enter(self, init_animations=False, single_instance=False):
        ...
        hud.elements.get(HUDElement.jack).set(self.shots.active.cue.theta)
        hud.elements.get(HUDElement.power).set(self.shots.active.cue.V0)

        tasks.register_event("space", self.shots.toggle_pause)
        tasks.register_event("arrow_up", self.shots.speed_up)
        tasks.register_event("arrow_down", self.shots.slow_down)
