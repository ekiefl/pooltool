...
class Resolver:
    ...
    def resolve(self, shot: System, event: Event) -> None:
        ...
        if event.event_type == EventType.NONE:
            ...
            _ = self.transition.resolve(ball, event.event_type, inplace=True)
        elif event.event_type == EventType.BALL_BALL:
            ball1 = shot.balls[ids[0]]
            ball2 = shot.balls[ids[1]]
            _ = self.ball_ball.resolve(ball1, ball2, inplace=True)
            ...
            _ = self.ball_linear_cushion.resolve(ball, cushion, inplace=True)
            ...
            _ = self.ball_circular_cushion.resolve(ball, cushion_jaw, inplace=True)
            ball.state.t = event.time
        elif event.event_type == EventType.BALL_POCKET:
            ball = shot.balls[ids[0]]
            pocket = shot.table.pockets[ids[1]]
            _ = self.ball_pocket.resolve(ball, pocket, inplace=True)
            ball.state.t = event.time
        elif event.event_type == EventType.STICK_BALL:
            cue = shot.cue
            ball = shot.balls[ids[1]]
            _ = self.stick_ball.resolve(cue, ball, inplace=True)
            ball.state.t = event.time
